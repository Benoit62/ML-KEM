...
func main() {
	...
	fmt.Println("Encapsulation Key:", encapsulationKey)
	fmt.Println("Decapsulation Key:", decapsulationKey)

	// Encapsulate a shared key.
	ciphertext, sharedKey, err := Encapsulate(encapsulationKey)
	...
	sharedKey2, err := Decapsulate(decapsulationKey, ciphertext)
	if err != nil {
		panic(err)
	}
	...
	if subtle.ConstantTimeCompare(sharedKey, sharedKey2) != 1 {
